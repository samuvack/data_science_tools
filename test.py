...
for pand in pand_list:
    cur.execute('INSERT INTO zimmo_kust_woningen VALUES (%s, %s, %s, %s, %s, %s,%s, %s,%s,%s,%s,%s,%s) ON CONFLICT (id) DO UPDATE SET oppervlakte = excluded.oppervlakte, slaapkamers=excluded.slaapkamers, prijs=excluded.prijs, adres=excluded.adres, postcode=excluded.postcode, gemeente=excluded.gemeente, epc = excluded.epc, lat = excluded.lat, lon = excluded.lon, bouwjaar = excluded.bouwjaar, woonopp = excluded.woonopp, grondopp = excluded.grondopp', (pand['id'], pand['grondopp'], pand['slaapkamers'], pand['prijs'], pand['adres'], pand['postcode'], pand['gemeente'], pand['epc'], pand['lat'], pand['lon'], pand['bouwjaar'], pand['woonopp'], pand['grondopp']))
conn.commit()
